...
#define foreach(it, c) for (/*__typeof__((c).begin())*/typename T::const_iterator it=(c).begin(); it != (c).end(); ++it)
template <typename T> void print_container(ostream& os, const T& c) { const char* _s = " "; if (!c.empty()) { /*__typeof__(c.begin())*/typename T::const_iterator last = --c.end(); foreach(it, c) { os << *it; if (it != last) os << _s; } } }
...
unsigned int __builtin_popcount(unsigned int x) { return __popcnt(x); }
#else
#include <sys/time.h>
...
Action beam_search(const InputInfo& input_info, ShadowKillJudger& shadow_kill_judger)
{
	...
	// ìGÇ™mpë´ÇËÇ»Ç¢éûÇ«Ç§Ç∑ÇÈÇ©ÇÕå„Ç≈åàÇﬂÇÈ
